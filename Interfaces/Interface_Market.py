...
def parse_transaction(line):
    ...
    objets_list2 = [item.strip()[:-2] if i == len(parts[4].strip('[]').split('|')) - 1 else item.strip() for i, item in
                    enumerate(parts[4].strip('[]').split('|'))]
...
class Application(tk.Tk):
    def __init__(self, filename):
        ...
        self.button_inventory = tk.Button(self, text="Inventaire", command=self.go_to_inventory)
        self.button_inventory.pack(side=tk.LEFT, padx=5)
        self.button_play_game = tk.Button(self, text="Jouer", command=self.play_game)
        self.button_play_game.pack(side=tk.LEFT, padx=5)
